...
{
    [SerializeField] [BoxGroup("Definitions")]
    private BubbleBehavior bubblePrefab; // Prefab da bolha

    [SerializeField] [BoxGroup("Definitions")]
    private Transform spawnPoint; // Ponto de spawn da bolha

    [SerializeField] [BoxGroup("Shoot Settings")]
    private float fireRate = 0.5f; // Taxa de disparo

    [SerializeField] [ReadOnly] [BoxGroup("Shoot Settings")]
    private float fireDelay; // Tempo do próximo disparo permitido

    [SerializeField] [ReadOnly] [BoxGroup("Bubble Settings")] [Dropdown("GetVectorValues")]
    private Vector2 bubbleDirection; // Direção da bolha

    private PlayerInput _input; // Sistema de Input
    private PlayerController _controller; // Controlador do jogador

    public PlayerController Controller => _controller = value;
}
